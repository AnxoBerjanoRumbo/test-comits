...
try {
    $stmt = $conexion->prepare("SELECT * FROM usuarios WHERE nick = :nick OR email = :nick");
    $stmt->execute([':nick' => $nick]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user) {
        // 1. Rate limiting: bloqueo de 1 minuto si > 5 fallos recientes
        if ($user['intentos_fallidos'] >= 5 && strtotime($user['ultimo_fallo']) > (time() - 60)) {
            header("Location: ../login.php?error=rate_limit");
            exit();
        }

        // 2. Email bloqueado permanentemente
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        $conexion->prepare("UPDATE usuarios SET intentos_fallidos = 0, ultimo_fallo = NULL WHERE id = :id")
                 ->execute([':id' => $user['id']]);

        // 3. Verificar baneo — regenerar sesión ANTES de escribir datos sensibles
        ...
        // Credenciales incorrectas — registrar intento fallido si el usuario existe
        if ($user) {
            $conexion->prepare("UPDATE usuarios SET intentos_fallidos = intentos_fallidos + 1, ultimo_fallo = NOW() WHERE id = :id")
                     ->execute([':id' => $user['id']]);
        }
